<!-- panel -->
<div class="panel">
	<div class="panel-heading">Ubah Data Disposisi Surat Masuk</div>
		...
		if(isset($_POST['simpan'])){
			...
			$tgl_disposisi = date_create($_POST['tgl_disposisi']);
			$tgl_disposisi2 = date_format($tgl_disposisi,'Y-m-d');
			$dari = $_POST['dari'];
			$kepada = $_POST['kepada'];
			...
			$tgl_terima2 = date_format($tgl_terima2,'Y-m-d');
			...
			$sql = mysql_query("update disposisi set kode_disposisi='$kode_disposisi',kode_sm='kode_sm',tgl_disposisi='$tgl_disposisi2',dari='$dari',kepada='$kepada',tgl_surat='$tgl_surat2',catatan_disposisi='$catatan_disposisi',status_terima='$status_terima',tgl_terima='$tgl_terima2',penerima='$penerima' where id='$uid'");
							if($sql > 0){
		...
		<?php
			...
			$sql = mysql_query("select * from disposisi where id='$id'");
			$rsql = mysql_fetch_array($sql); 
		?>	
		
		<!-- form isian -->
		<form name="frm" class="table-form" method="post" action="?q=disposisi-edit" enctype="multipart/form-data"> 
			<table>
