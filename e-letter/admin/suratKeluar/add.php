<!-- panel -->
<div class="panel">
	<div class="panel-heading">Tambah Surat Keluar</div>
		...
		if(isset($_POST['simpan'])){
			...
			if(empty($nomor_sk) || empty($tgl_terima) || empty($dari) || empty($no_surat) || empty($perihal) || empty($tujuan) ||empty($jenisSurat) || empty($_FILES["fileScan"]["tmp_name"])){
				 ...
				 echo"<div class=alert-error>".$error."</div>";
						...
						$sql = mysql_query("insert into surat_keluar (nomor_sk,tgl_terima,dari,no_surat,lampiran,file_scan,idjenis_surat,perihal,tujuan,keterangan,user_entry) values('$nomor_sk','$tgl_terima2','$dari','$no_surat','$lampiran','$file_scan','$idjenis_surat','$perihal','$tujuan','$keterangan','$user_entry')");
							if($sql > 0){
		...
		<!-- form isian -->
		<form name="frm" class="table-form" method="post" action="?q=suratk-add" enctype="multipart/form-data"> 
			<table>
